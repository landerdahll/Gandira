...
describeIntegration('Clube Outrahora checkout with real PostgreSQL', () => {
  ...
  const benefits = new ClubBenefitsService();
  ...
  async function reserve(prisma: PrismaClient, suffix: string) {
    try {
      return await withSerializableRetry(() => prisma.$transaction(async (tx) => {
        ...
        const order = await tx.order.create({ data: {
          ...
          discountAmount: decision.discountAmount!, total: new Prisma.Decimal(220).sub(decision.discountAmount!),
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        throw new ConflictException('reserva concorrente');
      }
      throw error;
    }
  }
  ...
  it('confirma o snapshot e associa o ticket do lote mais caro', async () => {
    ...
    const expiration = new OrderExpirationService(prismaA as never, benefits);
    const fulfillment = new OrderFulfillmentService(
      prismaA as never, new TicketsService(prismaA as never), mail as never, config as never, expiration, benefits,
    );
