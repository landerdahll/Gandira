...
describe('ClubMembersService', () => {
  ...
  function setup() {
    ...
    return { service: new ClubMembersService(prisma as never), prisma, tx };
  }
  ...
  it('rejeita CPF com checksum inválido antes de escrever', async () => {
    const { service, tx } = setup();
    await expect(service.create({ cpf: '111.111.111-11' }, 'admin-1')).rejects.toBeInstanceOf(BadRequestException);
