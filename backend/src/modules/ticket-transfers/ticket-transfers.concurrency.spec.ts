...
describe('ticket transfer concurrency claims', () => {
    ...
    const tx: any = {
      ticketTransfer: {
        findUnique: jest.fn(async ({ where }: any) => {
          if (where.invitationTokenHash && state.transfer.invitationTokenHash === null) return null;
          return { ...state.transfer, ticket: { ...state.ticket } };
        }),
        updateMany: jest.fn(async ({ where, data }: any) => {
          if (state.transfer.id !== where.id || state.transfer.status !== where.status) return { count: 0 };
          if (where.invitationTokenHash && state.transfer.invitationTokenHash === null) return { count: 0 };
      ...
      ticket: {
        updateMany: jest.fn(async ({ where, data }: any) => {
          if (state.ticket.id !== where.id || state.ticket.status !== where.status || state.ticket.ownerUserId !== where.ownerUserId) return { count: 0 };
          Object.assign(state.ticket, data);
          return { count: 1 };
        }),
      },
      ticketHistory: { createMany: jest.fn(async ({ data }: any) => state.histories.push(...data)) },
    };
    return { state, tx };
  }
  ...
  it('permite apenas uma conclusão para duas tentativas simultâneas do mesmo convite', async () => {
    ...
    const service = new TicketTransfersService({} as any, {} as any, {} as any);
  ...
  it('permite apenas um vencedor entre cancelamento e conclusão pelo cadastro', async () => {
    ...
    const mail: any = { sendTicketTransferEmail: jest.fn().mockResolvedValue(undefined) };
    const service = new TicketTransfersService(prisma, mail, {} as any);
    jest.spyOn(service, 'hashInviteToken').mockReturnValue('ignored');
    const completion = service.completeInviteInTransaction(
      tx,
      'raw',
      { id: 'recipient-1', name: 'Recipient', email: 'recipient@example.com' },
      { nextToken: 'next-token', qrCodeUrl: 'data:image/png' },
    );
  ...
  function activeTicketState() {
    const state: any = {
      ...
      transfers: [] as any[], checkIns: [] as any[], histories: [] as any[],
    };
    const tx: any = {
      ticket: {
        findUnique: jest.fn(async ({ where }: any) => {
          if (where.token && where.token !== state.ticket.token) return null;
          return { ...state.ticket, checkIn: state.checkIns[0] ?? null };
        }),
        updateMany: jest.fn(async ({ where, data }: any) => {
          if (state.ticket.id !== where.id || state.ticket.status !== where.status) return { count: 0 };
          if (where.ownerUserId && state.ticket.ownerUserId !== where.ownerUserId) return { count: 0 };
          if (where.token && state.ticket.token !== where.token) return { count: 0 };
          Object.assign(state.ticket, data);
          return { count: 1 };
        }),
      },
      user: { findUnique: jest.fn().mockResolvedValue(null) },
      ticketTransfer: { create: jest.fn(async ({ data }: any) => { const row = { id: `tr-${state.transfers.length + 1}`, ...data }; state.transfers.push(row); return row; }) },
      ticketHistory: {
        create: jest.fn(async ({ data }: any) => state.histories.push(data)),
        createMany: jest.fn(async ({ data }: any) => state.histories.push(...data)),
      },
      checkIn: { create: jest.fn(async ({ data }: any) => state.checkIns.push({ ...data, checkedAt: new Date() })) },
    };
    return { state, tx };
  }
  ...
  it('deixa apenas uma transferência em solicitações duplicadas concorrentes', async () => {
    const { state, tx } = activeTicketState();
    const prisma: any = { $transaction: jest.fn((callback: any) => callback(tx)) };
    const service = new TicketTransfersService(prisma, { sendTicketTransferEmail: jest.fn().mockResolvedValue(undefined) } as any, { get: jest.fn((_key: string, fallback: string) => fallback) } as any);
