...
describe('ClubBenefitsService', () => {
  const service = new ClubBenefitsService();
  const now = new Date('2026-07-22T20:00:00.000Z');

  function tx(options: { member?: any; usage?: any } = {}) {
    return {
      ...
      clubMember: { findUnique: jest.fn().mockResolvedValue(options.member === undefined ? {
        id: 'member-1', isActive: true, discountPercentage: new Prisma.Decimal('12.50'),
      } : options.member) },
      clubBenefitUsage: {
  ...
  const items = [
    { batchId: 'b-low', batchName: 'Comum', sortOrder: 0, quantity: 2, unitPrice: new Prisma.Decimal('80.00') },
    { batchId: 'b-high-2', batchName: 'VIP 2', sortOrder: 2, quantity: 1, unitPrice: new Prisma.Decimal('120.00') },
    { batchId: 'b-high-1', batchName: 'VIP 1', sortOrder: 1, quantity: 3, unitPrice: new Prisma.Decimal('120.00') },
  ];

  it('normaliza e-mail, escolhe uma unidade mais cara e desempata por sortOrder', async () => {
    const client = tx();
    const result = await service.evaluateInTransaction(client as never, {
  ...
  it.each([
    [null, 'NOT_MEMBER'],
    [{ id: 'member-1', isActive: false, discountPercentage: new Prisma.Decimal(10) }, 'INACTIVE_MEMBER'],
  ])('não aplica para membro ausente/inativo', async (member, reason) => {
    const result = await service.evaluateInTransaction(tx({ member }) as never, {
      userId: 'user-1', eventId: 'event-1', items, hasCoupon: false, now,
    });
  ...
  it('não consome benefício quando todos os ingressos são gratuitos', async () => {
    const result = await service.evaluateInTransaction(tx() as never, {
      userId: 'user-1', eventId: 'event-1',
      items: [{ ...items[0], unitPrice: new Prisma.Decimal(0) }], hasCoupon: false, now,
    });
    expect(result).toEqual({ applied: false, reason: 'NO_PAID_TICKETS' });
  });

  it('permite cupom quando o uso já foi confirmado', async () => {
    const result = await service.evaluateInTransaction(tx({ usage: { status: 'CONFIRMED' } }) as never, {
  ...
  it('rejeita cupom quando o benefício está disponível', async () => {
    await expect(service.evaluateInTransaction(tx() as never, {
      userId: 'user-1', eventId: 'event-1', items, hasCoupon: true, now,
