...
describe('UsersService.getProfile', () => {
    ...
    function setup(clubMember) {
        const prisma = {
            user: { findUnique: jest.fn().mockResolvedValue(user) },
            clubMember: { findUnique: jest.fn().mockResolvedValue(clubMember) },
        };
        return { service: new users_service_1.UsersService(prisma), prisma };
    }
    it('retorna membro ativo, apto e percentual como string decimal', async () => {
        const { service } = setup({ isActive: true, discountPercentage: new client_1.Prisma.Decimal('15.75') });
    ...
    it('normaliza espaços e caixa do e-mail antes de procurar o membro', async () => {
        const { service, prisma } = setup({ isActive: true, discountPercentage: new client_1.Prisma.Decimal('10') });
    ...
    it('mantém estrutura estável para membro inativo', async () => {
        const { service } = setup({ isActive: false, discountPercentage: new client_1.Prisma.Decimal('12.50') });
    ...
    it('não consulta membro quando o usuário não existe', async () => {
        ...
        await expect(service.getProfile('missing')).rejects.toBeInstanceOf(common_1.NotFoundException);
