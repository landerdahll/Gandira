...
const ticket_transfers_service_1 = require("../modules/ticket-transfers/ticket-transfers.service");
...
describeIntegration('Ticket transfers with real PostgreSQL Serializable transactions', () => {
    ...
    const sentMail = [];
    const mail = {
        sendTicketTransferEmail: jest.fn(async (to, _subject, _message, actionUrl) => {
    ...
    const transferA = new ticket_transfers_service_1.TicketTransfersService(prismaA, mail, config);
    const transferB = new ticket_transfers_service_1.TicketTransfersService(prismaB, mail, config);
    const ticketsA = new tickets_service_1.TicketsService(prismaA);
    ...
    beforeAll(async () => {
        ...
        const [identity] = await prismaA.$queryRaw `
    ...
    async function cleanupRunData() {
        ...
        const userWhere = { email: { startsWith: runPrefix } };
        if (eventIds.length) {
    ...
    async function createPendingInvite(fixture, suffix) {
    ...
    it('1. allows only one simultaneous transfer request for a ticket', async () => {
        ...
        const invoke = async (service, recipient) => {
            await start();
        ...
        expect(rejected?.reason).toBeInstanceOf(common_1.ConflictException);
    ...
    it('3. completes the same invite only once without an orphan account', async () => {
        ...
        const jwt = new jwt_1.JwtService({ secret: 'integration-secret' });
        const authA = new auth_service_1.AuthService(prismaA, jwt, config, mail, transferA);
        const authB = new auth_service_1.AuthService(prismaB, jwt, config, mail, transferB);
        const start = barrier(2);
        const register = async (service, name) => {
            await start();
    ...
    it('4. leaves one valid result for simultaneous cancellation and completion', async () => {
        ...
        const complete = async () => {
            await start();
            return (0, serializable_retry_util_1.withSerializableRetry)(() => prismaB.$transaction((tx) => transferB.completeInviteInTransaction(tx, invite.rawToken, recipient, { nextToken: `${fixture.key}-new`, qrCodeUrl: 'data:image/png;base64,new' }), { isolationLevel: client_1.Prisma.TransactionIsolationLevel.Serializable }));
        };
    ...
    it('6. rolls back every invited-registration write when invite completion fails', async () => {
        ...
        const auth = new auth_service_1.AuthService(prismaA, new jwt_1.JwtService({ secret: 'integration-secret' }), config, mail, transferA);
    ...
    it('provokes a real P2034, records retries, and never retries P2002', async () => {
        ...
        const update = (client, key, name) => (0, serializable_retry_util_1.withSerializableRetry)(async () => {
            firstAttempts[key] += 1;
            return client.$transaction(async (tx) => {
                await tx.user.findUniqueOrThrow({ where: { id: fixture.owner.id } });
                if (firstAttempts[key] === 1)
                    await firstRead();
        ...
        const nonRetryError = new client_1.Prisma.PrismaClientKnownRequestError('unique violation', { code: 'P2002', clientVersion: client_1.Prisma.prismaVersion.client });
        await expect((0, serializable_retry_util_1.withSerializableRetry)(async () => { nonRetryAttempts += 1; throw nonRetryError; })).rejects.toMatchObject({ code: 'P2002' });
...
//# sourceMappingURL=ticket-transfers.postgres.integration.spec.js.map